...
parser.add_argument(
    "--setup", action="store_true", default=False, help="Set up ledger's builtin roles"
)
...
parser.add_argument(
    "--aci-container-url", type=str, help="URL of attested aci container"
)
...
if __name__ == "__main__":
    ...
    with tempfile.NamedTemporaryFile(
        "w", suffix=".pem"
    ) as client_privk_file, tempfile.NamedTemporaryFile(
        "w", suffix=".pem"
    ) as client_cert_file, tempfile.NamedTemporaryFile(
        "w", suffix=".pem"
    ) as processor_privk_file, tempfile.NamedTemporaryFile(
        "w", suffix=".pem"
    ) as processor_cert_file:
        ...
        client_identity = test_harness._get_cert_identity(
            client_cert_file.name, client_privk_file.name
        )
        ...
        processor_identity = test_harness._get_cert_identity(
            client_cert_file.name, client_privk_file.name
        )
        ...
        with tester.client_set(
        ...
        ) as client, tester.client_set(
            endpoints=test_harness.endpoints,
            network_cert_file=test_harness.ca,
            session_auth=processor_identity,
        ) as processor_client, tester.client_set(
            endpoints=test_harness.endpoints,
            network_cert_file=test_harness.ca,
            session_auth=admin_cert_identity,
        ) as admin_client:
            ...
            resp = client.post("/app/cases", body=USER_INCIDENT)
